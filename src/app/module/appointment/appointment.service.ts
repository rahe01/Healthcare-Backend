...
const bookAppointment = async (payload: IBookAppointmentPayload, user: IRequestUser) => {
    ...
    const doctorSchedule = await prisma.dotcorSchedules.findUniqueOrThrow({
        where: {
            doctorId_scheduleId: {
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        await tx.dotcorSchedules.update({
            where: {
                doctorId_scheduleId: {
...
const canceledUnpaidAppointments = async () => {
