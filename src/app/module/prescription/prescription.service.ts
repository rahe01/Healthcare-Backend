...
const givePrescription = async (user: IRequestUser, payload: ICreatePrescriptionPayload) => {
    ...
    if (isAlreadyPrescribed) {
        throw new AppError(status.BAD_REQUEST, "Prescription already given for this appointment");
    }
        ...
        try {
            ...
            await sendEmail({
                ...
                templateData: {
                    ...
                    specialization: doctorData.specialties.map((s: any) => s.title).join(", "),
...
const myPrescriptions = async (user: IRequestUser) => {
    ...
    if (isUserExists.role === Role.PAITENT) {
        const prescriptions = await prisma.prescription.findMany({
            where: {
                patient: {
                    email: user?.email
